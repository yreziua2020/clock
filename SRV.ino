void server_init(void) {
 ...
 //TMP server.onNotFound([]() {(404, "text/plain", "FileNotFound");});
 #ifdef ESP8266
...
//======================================================================================================  
void handle_ConfigWeathJson() {  //zakat
...
void handle_ConfigSetupJson() {
  ...
  json += "\",\"Pu\":\"";
  json += p0;  //p0;  t0Lux;
  json += "\",\"pressSys\":\"";   ///в ччем выводить давление
  json += pressSys;
  json += "\",\"rotate\":\"";
  json += rotate;
  json += "\",\"NUM_MAX\":\"";
...
void handle_Set_Ssid() {
  ...
  if(printCom) {    printTime();    Serial.println("Set SSID: " + ssid + ",  Set password: " + password + ",  Set SSID AP: " + ssidAP + ",  Set AP password: " + passwordAP);  } 
  saveConfig();
...
void handle_ntp() {
  ...
  if(printCom) {      Serial.println( timeZone);  } 
...
//=============================================//обновляем с веба время но не пишем в RTC или пишем if(rtcStat) {hour_rtc=hour;minut====RRTTCC=====================================================
void handle_timepc() {
...
void handle_weather_update() {
  ...
  {
    if(printCom) {      printTime();      Serial.println("Update weather!");    }
    if(displayForecast) {
      if(!weatherHost){
        getWeatherData0();
        getWeatherDataz0();
...
void handle_weather() {
  ...
  if(printCom) {printTime();Serial.print("Set Weather Server: "); if(!weatherHost) Serial.print(weatherHost0); else Serial.print(weatherHost1);    Serial.println(", Key0: " + weatherKey0 + ", Key1: " + weatherKey1 + ", City ID0: " + cityID0 + ", City ID1: " + cityID1 + ", weatherLang: " + weatherLang);    Serial.println("displayForecast: " + String(displayForecast) + ", displayCityName: " + String(displayCityName) + ", displayForecastNow: " + String(displayForecastNow) + ", displayForecastTomorrow: " + String(displayForecastTomorrow));    Serial.println("outtimeScrollStart: " + String(timeScrollStart) + ", timeScrollStop: " + String(timeScrollStop) + ", timeScrollSpeed: " + String(timeScrollSpeed) + ", uuid: " + String(uuid) + ", api_key: " + String(api_key) + ", sensors_ID0: " + String(sensors_ID0) + ", sensors_ID1: " + String(sensors_ID1) + ", sensors_ID2: " + String(sensors_ID2));  }
  saveConfig();
  lang();
...
void handle_setup(){
  ...
  if(printCom) {    printTime();  Serial.println("TBD: "+String(timeDay)+", VBD: "+String(volBrightnessD)+", TBN: "+String(timeNight)+", VBN: "+String(volBrightnessN)+",  kuOn: "+String(kuOn)+",  kuOff: "+String(kuOff)+", rotate: "+String(rotate)+", clockNight: "+String(clockNight)+", buzzerSet: "+String(buzzerSet));    Serial.println("sensorDom: "+String(sensorDom)+", sensorUl: "+String(sensorUl)+", sensorHome: "+String(sensorHome)+", sensorHumi: "+String(sensorHumi)+",  sensorPrAl: "+String(sensorPrAl));  }
  saveConfig(); 
  server.send(200, "text/plain", "OK");
}
//======================================================================================================
void handle_mqtt_ust() {
  ...
  if(printCom) {     printTime();    Serial.println("Set mqtt_server: " + String(mqtt_server) + ",  mqtt_port: " + String(mqtt_port) + ",  mqtt_user: " + String(mqtt_user) + ",  mqtt_pass: " + String(mqtt_pass) + ", mqtt_name: " + String(mqtt_name));    Serial.println("          Set mqtt_sub1: " + String(mqtt_sub1) + ",  mqtt_sub2: " + String(mqtt_sub2) + ",  mqtt_sub3: " + String(mqtt_sub3)+ ",  mqtt_sub_inform: " + String(mqtt_sub_inform) + ",  mqtt_pub_temp: " + String(mqtt_pub_temp) + ",  mqtt_pub_tempUl: " + String(mqtt_pub_tempUl) + ",  mqtt_pub_hum: " + String(mqtt_pub_hum));  }
  //if(mqttOn) reconnect();
  saveConfig(); 
  server.send(200, "text/plain", "OK");
}
...
void handle_mqtt_on() {
  ...
  if(printCom) {    printTime();    Serial.println(server.arg("mqttOn")); }
  //if(mqttOn) reconnect();
  saveConfig(); 
  server.send(200, "text/plain", "OK");
}
...
void handle_resetConfig() {
  ...
  if(restart == "ok"){
    ...
    if(printCom) { printTime();  Serial.println("ESP erase Config file");    }
    delay(3000);
    server.send(200, "text/plain", "OK");
    delay(3000);
