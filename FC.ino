bool loadConfig() {                                                // Завантаження даних збереженних в файлі config.json
  ...
  if(printCom) {    printTime();    Serial.print("Load Config : ");    Serial.println(jsonConfig);  }
  return true;
}
//=================================================================
bool saveConfig() {
  ...
  jsonConfig = "";
  if(serializeJson(doc, jsonConfig)==0){ if (printCom) {Serial.println(F("Failed to write to jsonConfig"));} }  
...
bool loadTime() {
  ...
  if(!timeFile) {
    if(printCom) Serial.println("Failed to open time file");    saveTime();    timeFile.close();   return false;  }
  size_t size = timeFile.size();
  if(size > 512) {
    if(printCom) Serial.println("Time file size is too large");    timeFile.close();    return false;  }
  ...
  month = doc["month"];
  day = doc["day"]; 
  if(printCom) {    printTime();    Serial.print("Load Time : ");    Serial.println(jsonTime);  }
  return true;
}
//=================================================================
bool saveTime() {
  ...
  if(printCom) {    printTime();    Serial.print("Save Time: ");    Serial.println(jsonTime);  }
  timeFile.close();
  return true;
}
